...
class Orders extends CI_Controller {
  ...
  public function status($id)
  {
    ...
    else if ($status == 1)
    {
      ...
      foreach ($rowdetail as  $value) {
        ...
        $this->session->set_userdata('success', 'Cập nhật đơn hàng '.$row['orderCode'].' thành công !!');
      }
    ...
    {
      $this->session->set_userdata('error', 'Đơn hàng đã giao và thanh toán, không thể chỉnh sửa !');
      redirect('admin/orders','refresh');
    }
  ...
  public function trash($id)
  {
    ...
    $status = $row['status'];
    if($status == 1){
      $this->session->set_userdata('error', 'Đơn hàng '.$row['orderCode'].' đang được giao, không thể lưu !');
      redirect('admin/orders','refresh');
    }else{
      $mydata= array('trash' => 0);
      $this->Morders->orders_update($mydata, $id);
      $this->session->set_userdata('success', 'lưu đơn hàng '.$row['orderCode'].' vào thùng rác thành công');
      redirect('admin/orders','refresh');
    }
  }
  ...
  public function restore($id)
  {
    ...
    $this->session->set_userdata('success', 'Khôi phục đơn hàng thành công');
