...
class Useradmin extends CI_Controller {
	...
	public function insert(){
		...
		if ($this->form_validation->run() == TRUE){
			...
			$this->session->set_userdata('success', 'Thêm tài khoản thành công');
	...
	public function update($id){
		...
		$this->form_validation->set_rules('password', 'Mật khẩu', 'min_length[5]|max_length[32]');
		if ($this->form_validation->run() == TRUE) 
		{
			$mydata= array(
				'fullname' =>$_POST['fullname'], 
            ...
            if ($_POST['password']){
                $mydata['password'] = sha1($_POST['password']);
            }
         	$config = array();
	         $config['upload_path']   = 'public/images/admin/';
			...
			$this->session->set_userdata('success', 'Cập nhật tài khoản thành công');
            if ($user_role['id'] == $id) {
                $row=$this->Muser->user_detail_id($id);
	...
	public function trash($id)
	{
		$row=$this->data['user'];
		if($row['role']==2){
			$this->session->set_userdata('error', 'Không có quyền thao tác');
			redirect('admin/useradmin','refresh');
		}else{
			$mydata= array('trash' => 0);
			$this->Muser->user_update($mydata, $id);
			$this->session->set_userdata('success', 'Xóa tài khoản vào thùng rác thành công');
			redirect('admin/useradmin','refresh');
		}
	}
	...
	public function status($id)
	{
		...
		if($user['role']==2){
			...
			if($id!=$user['id']){
				...
				$this->session->set_userdata('error', 'Không thể thao tác với chính mình');
				redirect('admin/useradmin','refresh');
			}
		}
	}
	...
	public function restore($id){
		...
		if($row['role']==2){
			...
			$this->session->set_userdata('success', 'Khôi phục tài khoản thành công');
	...
	public function delete($id){
		...
		if($row['role']==2){
			...
			$this->session->set_userdata('success', 'Xóa tài khoản thành công');
			redirect('admin/useradmin/recyclebin','refresh');
		}
	}
