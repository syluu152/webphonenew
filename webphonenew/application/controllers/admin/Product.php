      ...
      redirect('admin/E403/index','refresh');
	    ...
	    //      //Chiều cao tối đa
	        ...
	        //Lưu nhóm hình ảnh chi tiết
              ...
              $this->session->set_userdata('success', 'Thêm sản phẩm thành công');
         ...
         if ($this->form_validation->run() == TRUE){
          $mydata= array(
           ...
           'option'=>$_POST['option-product'],
              ...
              $this->session->set_userdata('success', 'Cập nhật sản phẩm thành công');
              redirect('admin/product','refresh');
            } 
...
//             if(count($row) > 0){
//                 $this->session->set_userdata('error', 'Đã có khách hàng đặt mua, không thể xóa !');
//                 redirect('admin/product','refresh');
//             }
            $mydata= array('trash' => 0,'modified_by'=>$this->session->userdata('id'),);

            $this->Mproduct->product_update($mydata, $id);
            $this->session->set_userdata('success','Xóa sản phẩm vào thùng rác thành công');
        ...
        public function restore($id){
         ...
         $this->session->set_userdata('success', 'Khôi phục sản phẩm thành công');
       ...
       public function delete($id){
         ...
         $this->session->set_userdata('success', 'Xóa sản phẩm thành công');
         redirect('admin/product/recyclebin');
       }
