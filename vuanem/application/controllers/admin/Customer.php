...
class Customer extends CI_Controller {
	...
	public function trash($id)
	{
		...
		if(count($row) > 0)
		{
			$this->session->set_userdata('error', 'Khách hàng đã đặt hàng, không thể xóa !');
		...
		else
		{
			...
			$this->session->set_userdata('success', 'Xóa khách hàng vào thùng rác thành công');
			redirect('admin/customer','refresh');
		}
	}
	public function restore($id)
	{
		...
		$this->session->set_userdata('success', 'Khôi phục khách hàng thành công');
	...
	public function delete($id)
	{
		...
		$this->session->set_userdata('success', 'Xóa khách hàng thành công');
		redirect('admin/customer/recyclebin','refresh');
	}
