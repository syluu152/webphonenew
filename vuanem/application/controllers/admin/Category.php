...
class Category extends CI_Controller {
	...
	public function insert()
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			$this->session->set_userdata('success', 'Thêm danh mục thành công');
	...
	public function update($id)
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			$this->session->set_userdata('success', 'Cập nhật danh mục thành công');
			redirect('/admin/category','refresh');
		} 
	...
	public function status($id)
	{
		...
		$this->Mcategory->category_update($mydata, $id);
		$this->session->set_userdata('success', 'Cập nhật loại sản phẩm thành công');
		redirect('admin/category','refresh');
	}
	...
	public function trash($id)
	{
		...
		if($count_product > 0)
		{
			$this->session->set_userdata('error', 'Danh mục này còn sản phẩm bên trong! Hãy xóa sản phẩm trước !');
			redirect('admin/category','refresh');
		}
		else
		{
			if($count_category > 0)
			{
				$this->session->set_userdata('error', 'Danh mục này còn danh mục con bên trong! Không thể thực hiện !');
				redirect('admin/category','refresh');
			}
			else
			{
				...
				$this->Mcategory->category_update($mydata, $id);
				$this->session->set_userdata('success', 'Xóa loại sản phẩm vào thùng rác thành công');
				redirect('admin/category','refresh');
			}
		}
	}
	...
	public function restore($id)
	{
		...
		$this->session->set_userdata('success', 'Khôi phục loại sản phẩm thành công');
	...
	public function delete($id)
	{
		...
		$this->session->set_userdata('success', 'Xóa loại sản phẩm thành công');
		redirect('admin/category/recyclebin','refresh');
	}
